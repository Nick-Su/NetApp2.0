        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
               ...
               // var newinfo = await AuthenticationManager.User;
                ...
                {
                   // var loginInfo = await AuthenticationManager.GetExternalLoginInfoAsync();
                    firstNameClaim = info.ExternalIdentity.Claims.First(c => c.Type == "urn:facebook:first_name").ToString();
                    lastNameClaim = info.ExternalIdentity.Claims.First(c => c.Type == "urn:facebook:last_name").ToString();
                    var fbuid = info.ExternalIdentity.Claims.First(c => c.Type == "urn:facebook:id").ToString();

                    // To get (download user picture)
                    /*  https://graph.facebook.com/1380057325498670?fields=picture.width(720).height(720) */
                    //   http://graph.facebook.com/1380057325498670/picture?type=large
                }
                ...
                if (info.Login.LoginProvider == "VKontakte")
                {
                    ...
                    firstNameClaim = words.First();
                    lastNameClaim = words.Last();
                }
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FirstName = firstNameClaim, LastName = lastNameClaim };
