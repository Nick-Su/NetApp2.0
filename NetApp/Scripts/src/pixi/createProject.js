...
//for (let i = 0; i < texturesButton.length; i++) {
...
addButton = new PIXI.Sprite(texturesButton[0]);

addButton.anchor.set(0.5);
addButton.x = buttonPositions[0 * 2];
addButton.y = buttonPositions[0 * 2 + 1];
...
function addProject()
{
    nameRes = document.getElementById('nameProject');
    whatDo = document.getElementById('whatDoProject');
    whom = document.getElementById('whom');
    feature = document.getElementById('feature');
    //choosedBtn.tint = tintBefor;
    window.opener.createRectangle(nameProject.value, whatDo.value, whom.value, feature.value);
    window.close();
}
